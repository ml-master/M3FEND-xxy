...
# from models.m3fend_wosem import Trainer as M3FENDTrainer
# from models.m3fend_wosemo import Trainer as M3FENDTrainer
# from models.m3fend_wosty import Trainer as M3FENDTrainer
# from models.m3fend_wointer import Trainer as M3FENDTrainer
from models.m3fend_womeo import Trainer as M3FENDTrainer
# from models.m3fend_woada import Trainer as M3FENDTrainer
...
class Run():
    def __init__(self,
        ...
        self.category_dict = config['category_dict'] # 具体的元素数量跟domain_num参数的值有关
    ...
    def main(self):
        ...
        if self.model_name == 'textcnn':
            ...
            trainer = M3FENDTrainer(emb_dim = self.emb_dim, mlp_dims = self.mlp_dims, use_cuda = self.use_cuda, lr = self.lr, train_loader = train_loader, dropout = self.dropout,
                                    weight_decay = self.weight_decay, val_loader = val_loader, test_loader = test_loader, category_dict = self.category_dict,
                                    early_stop = self.early_stop, epoches = self.epoch, save_param_dir = os.path.join(self.save_param_dir, self.model_name),
        ...
        # """添加创建相应文件夹的代码
        # [Errno 2] No such file or directory: './logs/json/m3fend.json'
        # """
        ...
        for p, vs in param.items():
            ...
            for i, v in enumerate(vs):
                # vs=[0.0001, 0.0001, 0.0001, 0.0001, 0.0001, 0.0001, 0.0001, 0.0001, 0.0001, 0.0001]
                """setattr(trainer, p, v) 的作用是在 trainer 对象上设置一个名为 p 的属性,并将其值设置为 v。这相当于执行 trainer.p = v。
                    通过使用 setattr() 函数,你可以动态地为 trainer 对象设置属性,而不需要显式地写出每个属性的赋值语句。"""
                setattr(trainer, p, v)
