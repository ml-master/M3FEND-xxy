...
class M3FENDModel(torch.nn.Module):
    ...
    def forward(self, **kwargs):
        ...
        # 假设原始的 shared_feature 形状为 (64, 16, 320)
        original_shape = shared_feature.shape
        target_shape = (original_shape[0], 50, original_shape[2])  # 目标形状 (64, 50, 320)
        # 创建一个新的全零张量
        new_shared_feature = torch.zeros(target_shape, device=shared_feature.device, dtype=shared_feature.dtype)
        # 将原始数据复制到新张量的前16行
        new_shared_feature[:, :original_shape[1], :] = shared_feature

        # 直接使用新创建的张量
        shared_feature = new_shared_feature

        # shared_feature 现在的形状是 (64, 50, 320)，但只有前16行包含原始数据，其余为0
