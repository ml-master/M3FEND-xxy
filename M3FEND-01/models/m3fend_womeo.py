...
class SimplifiedMemoryNetwork(torch.nn.Module):
    def __init__(self, input_dim, emb_dim, domain_num, memory_num=10):
        super(SimplifiedMemoryNetwork, self).__init__()
        self.domain_num = domain_num
        self.emb_dim = emb_dim
        self.memory_num = memory_num # 每个domain的event memory矩阵存放的all_feature数量（聚类的数量）
        self.tau = 32
    ...
    def forward(self, feature, category):
        ...
        # 生成全 1 的注意力权重
        uniform_att = torch.ones(batch_size, self.domain_num).cuda()
        # 归一化以确保每行的和为 1
        domain_att = uniform_att / self.domain_num
        domain_att = domain_att.unsqueeze(1)  # 添加一个维度以匹配原始输出
...
class M3FENDModel(torch.nn.Module):
    def __init__(self, emb_dim, mlp_dims, dropout, semantic_num, emotion_num, style_num, LNN_dim, domain_num,dataset):
        ...
        if dataset == 'ch':
            self.domain_memory = SimplifiedMemoryNetwork(input_dim = self.emb_dim + 47 * 5 + 48, emb_dim = self.emb_dim + 47 * 5 + 48, domain_num = self.domain_num, memory_num = self.memory_num)
        elif dataset == 'en':
            self.domain_memory = SimplifiedMemoryNetwork(input_dim = self.emb_dim + 38 * 5 + 32, emb_dim = self.emb_dim + 38 * 5 + 32, domain_num = self.domain_num, memory_num = self.memory_num)
    ...
    def forward(self, **kwargs):
        ...
        news_feature=torch.cat([gate_input_feature, emotion_feature, style_feature], dim = -1)
        memory_att = self.domain_memory(news_feature, category)
