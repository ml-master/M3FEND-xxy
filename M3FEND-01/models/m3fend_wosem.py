...
class M3FENDModel(torch.nn.Module):
    def __init__(self, emb_dim, mlp_dims, dropout, semantic_num, emotion_num, style_num, LNN_dim, domain_num,dataset):
        ...
        print('emotion_num_expert:', self.emotion_num_expert, 'style_num_expert:', self.style_num_expert, 'lnn_dim:', self.LNN_dim)
        self.fea_size =256
        self.emb_dim = emb_dim
        if dataset == 'ch':
    ...
    def forward(self, **kwargs):
        ...
        for i in range(self.semantic_num_expert):
            ...
            # 创建一个与原始结果形状相同但值全为0的张量--抹除掉semnet提取的语义特征
            zero_tensor = torch.zeros_like(self.content_expert[i](content_feature))
            shared_feature.append(zero_tensor.unsqueeze(1))  # r_sem
        for i in range(self.emotion_num_expert):
